...
const appnd = '&append_to_response=credits';
...
async function malabar(x) {
    try {
        ...
        idcoll.forEach(e =>    {                   //5 LOOPING THROUGH EACH ID
            sabar(burl + `/movie/${e}?&` + apikey + appnd)
           }
            );
    } catch (error) {
...
async function sabar(x) {
    try {
        ...
        const direc = tj.credits.crew.find(member => member.job === 'Director');
        ...
        const dj =console.log(direc.name);
        dfd.innerHTML= 
    `<div class="one">
    <div class="three"><span class="plot">${mj.overview}</span></div>
    <div class="to">
        <img src="${imgurl+mj.backdrop_path}" alt="${id}" onerror="this.onerror=null;this.src='mmm.jpg;'>
    </div>
    <div class="textblock">
        <p class="title">${mj.title}</p>
      <p class="crd"><span class="year">${direc.name}</span><span
